/**
 * 数据源：Qoder（阿里）
 * 用量缓存在 VS Code secret storage（secret://aicoding.auth.creditUsage），
 * best-effort 解密后递归提取含 tokens 字段的对象。失败时 healthy=false。
 */
const path = require('path');
const os = require('os');
const { readSecret } = require('./vscode-secret');

const DEFAULT_ROOT = path.join(os.homedir(), 'AppData', 'Roaming', 'Qoder');
const SECRET_KEY = 'aicoding.auth.creditUsage';
...
function rebuild() {
  const root = process.env.QODER_DIR || DEFAULT_ROOT;
  ...
  try {
    ...
    if (!plain) {
      cache.healthy = false;
      cache.reason = '解密失败';
      console.warn('[qoder] 用量缓存解密失败（加密格式或密钥不可用）');
    } else {
      let data = null;
      try { data = JSON.parse(plain); } catch { /* 明文非 JSON */ }
      if (data && typeof data === 'object') {
        extractRecords(data, records, 'root');
      }
      // 解密可用但只有额度配额（credits），无 token 明细 → 不算解密失败
      cache.healthy = true;
      cache.reason = records.length ? '' : '缓存仅含配额无用量明细';
    }
  } catch (e) {
    cache.healthy = false;
    cache.reason = '解密失败';
    console.warn('[qoder] 解析失败:', e.message);
  }
...
module.exports = { getRows, status, source: 'qoder' };
